...
The $\gamma$ ratio between 3L and 4L is 24$\times$ (Field 15) to 32$\times$ (Field 12), confirming that automatic search is essential (Figure~\ref{fig:autogamma}).

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/auto_gamma_comparison.pdf}
\caption{Auto-$\gamma$ results. Left: cluster count decreases with 4L (Emb addition reduces over-fragmentation). Right: average cluster size increases, especially in Field~12 ($2.6\times$).  Auto-$\gamma$ adjusts resolution by 24--32$\times$ to maintain balanced distributions.}
\label{fig:autogamma}
\end{figure}
...
Figure~\ref{fig:combo} decomposes the consensus by specific layer combinations.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/consensus_combo_distribution_f15.pdf}
\caption{Edge count by consensus combination. Among 2-layer pairs, BC+CC dominates (3.6\%); among 3-layer, BC+CC+Emb and BC+CC+DC are comparable. Only 240 edges (0.07\%) achieve full 4-layer consensus.}
\label{fig:combo}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{figures/layer_overlap_heatmap_f15.pdf}
\caption{Pairwise overlap rate (\% of smaller layer's edges). BC$\cap$CC has the highest overlap (16\%), while Emb overlaps least with citation layers ($<$7.3\%).}
\label{fig:overlap}
\end{figure}

Key patterns:
\begin{itemize}[nosep]
    \item \textbf{BC+CC is the strongest pair} (12{,}947 edges, 3.6\%) --- papers sharing references tend to also be co-cited.
    \item \textbf{Emb is the most independent layer} --- lowest pairwise overlap with all citation layers ($<$7.3\%), confirming it captures complementary information.
    \item \textbf{3-layer combinations involving BC+CC dominate} --- BC+CC+Emb (1{,}479) and BC+CC+DC (1{,}353) are the most common triplets.
    \item \textbf{Full consensus is rare but informative} --- only 240 edges achieve 4-layer agreement, but these form the densest cluster cores.
\end{itemize}

\subsection{Embedding as Complementary Signal}

Although Emb overlaps least with citation layers ($<$7.3\%), its independent edges are not noise---they capture complementary information (Figure~\ref{fig:emb}).

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/emb_contribution_f15.pdf}
\caption{Embedding layer contribution. Left: 73.5\% of Emb-only edges connect same-subfield papers (vs. 98.4\% for citation). Center: Emb bridges wider temporal gaps (mean 3.8yr vs. 1.5yr). Right: BC is Emb's strongest consensus partner (4{,}826 edges).}
\label{fig:emb}
\end{figure}

Emb provides two unique capabilities:
\begin{enumerate}[nosep]
    \item \textbf{Cross-subfield bridges}: 26.5\% of Emb-only edges cross subfield boundaries (vs. 1.6\% for citation), connecting topically related work across disciplinary lines.
    \item \textbf{Temporal bridging}: Mean year gap of 3.8 years (vs. 1.5 for citation) connects recent papers to earlier related work not yet reflected in citation patterns.
\end{enumerate}
...
\subsection{Field Comparison: Citation-Rich vs. Citation-Poor}

Repeating the analysis on Field~12 (Arts \& Humanities, 754K papers) reveals strikingly different patterns (Figure~\ref{fig:f12_landscape}).

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/edge_landscape_f12.pdf}
\caption{Field 12 edge landscape. DC is extremely broad (35\% diagonal); Emb is the \emph{most} diagonal-concentrated (71\%), reversing the Field~15 pattern.}
\label{fig:f12_landscape}
\end{figure}

\begin{table}[h]
\centering
\caption{Cross-field comparison of consensus and embedding properties.}
\begin{tabular}{lrr}
\toprule
Metric & Field 15 (Chem.~Eng.) & Field 12 (Arts \& Hum.) \\
\midrule
\multicolumn{3}{l}{\emph{Temporal concentration (diagonal $\pm$3yr)}} \\
\quad BC & 78\% & 68\% \\
\quad CC & 74\% & 63\% \\
\quad DC & 44\% & 35\% \\
\quad Emb & 62\% & \textbf{71\%} \\
\midrule
\multicolumn{3}{l}{\emph{Consensus distribution}} \\
\quad 2-layer & 8.1\% & 2.5\% \\
\quad 3-layer & 1.0\% & 0.1\% \\
\quad Strongest 2L pair & BC+CC (13K) & BC+Emb (4.8K) \\
\midrule
\multicolumn{3}{l}{\emph{Embedding contribution}} \\
\quad Emb-only cross-subfield & 26.5\% & \textbf{56.1\%} \\
\quad Citation cross-subfield & 1.6\% & 19.0\% \\
\quad Emb year gap & 3.8yr & 3.0yr \\
\bottomrule
\end{tabular}
\end{table}
...
Key differences:
\begin{itemize}[nosep]
    \item \textbf{Emb dominance reversal}: In humanities, Emb is the most temporally concentrated layer (71\%), because citation layers are too sparse to form tight connections.
    \item \textbf{Consensus rarity}: Only 2.5\% of edges achieve 2-layer consensus (vs. 8.1\% in engineering), making consensus weighting less differentiating.
    \item \textbf{Emb as CC substitute}: The strongest 2-layer pair shifts from BC+CC to BC+Emb --- embedding replaces co-citation in citation-poor fields.
    \item \textbf{Cross-disciplinary}: 56\% of Emb-only edges cross subfield boundaries (vs. 26.5\%), reflecting the inherently interdisciplinary nature of humanities.
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/consensus_levels_year_f12.pdf}
\caption{Field 12 consensus levels. Even 1-layer edges are highly diagonal (91\%) due to sparse, temporally narrow citation patterns. 3-layer achieves 98\% but contains only 458 edges.}
\label{fig:f12_consensus}
\end{figure}

%% ─────────────────────────────────────────────────────────
\subsection{Top-$k$ Filter Effect}

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{figures/topk_before_after.pdf}
\caption{Effect of per-node top-30 filter on edge composition. Without filtering, 70\% of combined edges are cross-cluster noise; with top-30, this drops to 31\%.}
\label{fig:topk}
\end{figure}

\subsection{LLM Blind Evaluation Summary}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/llm_evaluation_summary.pdf}
\caption{55-case LLM evaluation across three criteria. Consensus wins belonging (especially in large clusters), while cohesion and outlier counts are statistically equivalent.}
\label{fig:llm}
\end{figure}

\subsection{Field 12: Embedding Contribution and Consensus Structure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/emb_contribution_f12.pdf}
\caption{Field 12 embedding: 56\% cross-subfield (vs.\ 27\% in Field~15), BC+Emb is the dominant 2-layer pair (replacing BC+CC).}
\label{fig:emb_f12}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/consensus_combo_f12.pdf}
\caption{Field 12 consensus combinations: 97.4\% single-layer, BC+Emb dominates 2-layer. Contrast with Field~15 where BC+CC leads.}
\label{fig:combo_f12}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/cross_field_summary.pdf}
\caption{Head-to-head comparison of all key metrics between citation-rich (Field~15) and citation-poor (Field~12) fields.}
\label{fig:crossfield}
\end{figure}

\subsection{Clustering Stability}

We evaluate partition stability by running Leiden with 5 different random seeds at $\gamma=5.62\times10^{-5}$ on the consensus 3-layer network.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/stability_ami_ari.pdf}
\caption{Pairwise stability across 5 seeds. AMI $= 0.902 \pm 0.009$ indicates highly reproducible cluster structure; ARI $= 0.740 \pm 0.093$ reflects moderate variation in small clusters.}
\label{fig:stability}
\end{figure}

\subsection{Example Cluster Labels}

Table~\ref{tab:labels} shows the top clusters produced by the consensus pipeline on Field~15, with automatically extracted keyword labels.

\begin{table}[h]
\centering
\caption{Top 10 cluster labels from the consensus pipeline (Field~15, 4-layer, 58 clusters).}
\label{tab:labels}
\begin{tabular}{rlrl}
\toprule
...
\begin{itemize}[nosep]
    \item Consensus weighting assigns boundary nodes more accurately than simple summation (29:26 in 55 LLM blind reviews), with equivalent internal cohesion.
    \item The consensus mechanism works because 6.5\% of 3-layer edges receive $3\times$ weight, forming temporally coherent cluster cores (95\% within $\pm$3 years).
    \item Embedding adds unique value: 26--56\% cross-subfield bridging and 3--4 year temporal bridging that citation layers cannot provide.
    \item The pipeline is stable (AMI $= 0.902$) and scales from citation-rich to citation-poor fields via auto-$\gamma$ adaptation.
\end{itemize}

\subsection{Limitations}

\begin{itemize}[nosep]
    \item \textbf{Two fields only}: Validated on Chemical Engineering and Arts \& Humanities. Generalization to all 26 OpenAlex fields requires further testing.
    \item \textbf{Fixed top-$k{=}30$}: Optimal per-node neighbor count may vary by field density. A data-driven $k$ selection method is desirable.
    \item \textbf{Embedding quality}: SPECTER2 embeddings may introduce stylistic rather than topical similarity for short/non-English abstracts. The pipeline's effectiveness depends on embedding quality.
    \item \textbf{LLM evaluation}: GPT-4o-mini evaluations are approximate; human expert validation would strengthen the findings.
    \item \textbf{Static snapshot}: Analysis uses a single temporal snapshot. Temporal evolution of consensus structure is not yet studied.
\end{itemize}

\subsection{Future Work}

\begin{itemize}[nosep]
    \item \textbf{Field generalization}: Test on all 26 OpenAlex fields with varying citation densities and interdisciplinarity levels.
    \item \textbf{Adaptive top-$k$}: Set $k$ per-layer based on local degree distribution rather than a fixed constant.
    \item \textbf{Temporal tracking}: Monitor how consensus backbone evolves as new papers are added, enabling research trend detection.
    \item \textbf{Hierarchical keyword extraction}: Cascade cluster labels from nano to macro, ensuring parent labels generalize child labels.
    \item \textbf{Interactive web tool}: SciScape web interface with real-time OpenAlex queries, consensus visualization, and label editing.
\end{itemize}
