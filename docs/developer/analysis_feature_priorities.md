...
3. Maintain exposure states and manifest-backed feature fields in result-root
   validation.
4. Extend long-run controls beyond visible recovery metadata: in-flight HTTP
   interruption, robust retry policy, quota handling, in-app resume execution,
   and shard-aware operator controls.
