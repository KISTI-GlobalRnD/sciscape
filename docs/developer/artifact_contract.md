...
| `narrative/generation_metadata.json` | `schema_version`, `narrative_id`, `generation_mode`, `text_origins`, `llm_generation_used`, `model_generation`, `parameters`, `transforms`; optional `updated_claims` | Provenance for deterministic or model-assisted narrative text generation. |
| `narrative/generation_prompts/prompt_batch_manifest.json` | `schema_version`, `prompt_batch_id`, `narrative_id`, `prompt_ref`, `selection_policy`, `outputs`, `counts` | Provider-neutral prompt batch manifest for auditable model-assisted claim rewriting. |
| `narrative/generation_prompts/prompt_jobs.jsonl` | JSONL rows with `schema_version`, `job_id`, `claim_id`, `prompt_ref`, `prompt_digest`, `evidence_refs`, `prompt`, `expected_output` | Prompt jobs that can be sent to a model provider and later applied through `sciscape narrative apply-generated`. |
| `narrative/generation_outputs/generation_run_manifest.json` | `schema_version`, `prompt_batch_id`, `model_run_id`, `provider`, `model`, `source_prompt_manifest`, `outputs`, `counts`, `output_digest`; optional `apply_result` | Provider execution run manifest for generated narrative claim updates. |
| `narrative/generation_outputs/generated_claims.jsonl` | JSONL rows with `schema_version`, `job_id`, `claim_id`, `claim_text`, `provider`, `model`, `model_run_id`, `prompt_ref`, `prompt_digest`, `status` | Generated claim-update rows that can be passed to `sciscape narrative apply-generated`. |
