...
| F11 | Temporal and evolution | `[~]` | 72% | temporal keyword utilities, burst/trend helpers, feature detection, temporal/evolution artifact designs, temporal writer/validator, standalone membership-projection evolution analysis module, evolution writer/validator, synthetic evolution smoke, explicit document-overlap transition evidence builder, artifact-backed web Evolution lens, lineage-time map payload/UI | real-data richer matching writer and interaction polish |
...
- `[~]` Membership-projection evolution writer intentionally emits only events
  ...
  by the validator smoke and document-overlap evidence primitive, but still need
  a real-data slice-local membership writer before being promoted as default
  claims.
