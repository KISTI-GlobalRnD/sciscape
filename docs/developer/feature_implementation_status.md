...
| F12 | Evidence-backed narratives | `[~]` | 74% | narrative feature detection, target definition, evidence-reference artifact design, cluster review packet writer/validator, deterministic claim graph writer, model-assisted claim update hook, generation metadata artifact, claim/evidence validator, result-manifest beta/block exposure, job/cluster narrative API, Atlas narrative review block, review-decision writeback, latest-review readback, cluster review progress counts, review save/failure feedback, reviewed JSON/Markdown/HTML publication summary artifacts, reviewed publication cluster index, reviewed publication zip bundle | stable generator runner and polished reviewed publication/report workflow |
...
- `[ ]` There is no stable narrative generation runner or prompt execution
  pipeline.
...
feature until a stable generation runner, prompt execution contract, and the
