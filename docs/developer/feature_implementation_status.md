...
| F12 | Evidence-backed narratives | `[~]` | 62% | narrative feature detection, target definition, evidence-reference artifact design, cluster review packet writer/validator, deterministic claim graph writer, claim/evidence validator, result-manifest beta/block exposure, job/cluster narrative API, Atlas narrative review block, review-decision writeback, latest-review readback, cluster review progress counts, review save/failure feedback, reviewed Markdown/JSON publication summary artifact | stable generator and polished reviewed publication/report surface |
...
Status: `[~]` Partial. Rough completeness: 62%.
