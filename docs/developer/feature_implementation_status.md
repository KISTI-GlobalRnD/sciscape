...
- CLI: `query`, `cluster`, `keywords`, `convert`, `landscape`, `visualize`,
  ...
  `evolution-from-slice-membership`, `evolution`, `export`, `rule-export`,
  `matrix`, `bundle`, `web`, and `gui` commands exist in `sciscape/cli.py`.
...
- `[~]` Membership-projection evolution writer intentionally emits only events
  ...
  evidence pack, and one-shot membership-to-evolution writer, but full
  slice-local reclustering remains a future integration step.
- `[~]` Full evolution map layout is inspectable but not product-ready; it still
  needs richer time-slice matching and larger-scale layout polish.
...
1. Integrate default slice-local reclustering or reviewed periodized
   slice-membership evidence into the main evolution pipeline, then promote the
   Evolution lens into a true map layout.
