...
| F12 | Evidence-backed narratives | `[~]` | 56% | narrative feature detection, target definition, evidence-reference artifact design, cluster review packet writer/validator, deterministic claim graph writer, claim/evidence validator, result-manifest beta/block exposure, job/cluster narrative API, Atlas narrative review block, review-decision writeback, latest-review readback, review save/failure feedback | stable generator and reviewed publication surface |
...
- `[x]` Atlas narrative claims can receive review decisions from the web UI/API;
  ...
  narrative payload, narrative validation is refreshed, and the inspector shows
  pending/saved/failed feedback.
