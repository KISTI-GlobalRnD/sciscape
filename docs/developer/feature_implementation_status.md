...
| F12 | Evidence-backed narratives | `[~]` | 76% | narrative feature detection, target definition, evidence-reference artifact design, cluster review packet writer/validator, deterministic claim graph writer, model-assisted claim update hook, model-generated claim batch apply CLI, generation metadata artifact, claim/evidence validator, result-manifest beta/block exposure, job/cluster narrative API, Atlas narrative review block, review-decision writeback, latest-review readback, cluster review progress counts, review save/failure feedback, reviewed JSON/Markdown/HTML publication summary artifacts, reviewed publication cluster index, reviewed publication zip bundle | prompt execution runner and polished reviewed publication/report workflow |
...
- `[ ]` There is no stable prompt execution runner or end-to-end generation
  pipeline.
...
feature until a stable prompt execution runner, generation contract, and the
