...
| F09 | Atlas map, evidence, cluster reading | `[~]` | 94% | atlas payload builder, neighbors, representative works with citation copy metadata, compact source facets, web endpoints, evidence inspector model, review checklist, readiness reasons, persisted cluster review packet, filterable review queue, render payload adapter, split atlas-render endpoints, deck.gl prototype, layer controls, render/perf/interaction/inspector smoke gates | complete evidence review workflow |
...
- `[x]` Representative work rows preserve compact citation metadata and expose
  ...
  fields.
- `[x]` Joined record source/venue fields can populate bounded `top_sources`
  evidence rows for selected clusters without promoting sources to normalized
  entities.
...
- `[x]` The inspector now includes a compact review checklist that summarizes
  term, work, relation, and QA readiness for cluster interpretation, including
...
- `[x]` Model-assisted claim text can be applied through a safe writer hook that
  updates declared claim ids only, preserves the existing claim/evidence link
  table, records model/prompt metadata, resets generated claims to a
  review-required state, and refreshes narrative QA.
- `[x]` `sciscape narrative render-prompts` can render provider-neutral JSONL
  prompt jobs from validated claim/evidence artifacts, including claim ids,
  prompt digests, target context, and evidence refs.
- `[x]` `sciscape narrative run-prompts` can execute rendered prompt jobs through
  an explicit provider runner, write generated claim-update JSONL and a run
  manifest, and optionally apply successful rows through the safe update hook.
- `[x]` `sciscape narrative apply-generated` can apply JSON or JSONL generated
  claim updates through that safe writer hook, making batch model outputs
  auditable after prompt execution.
...
- `[x]` Atlas narrative claims can receive review decisions from the web UI/API;
  ...
  is updated, latest review decisions are exposed back through the cluster
...
- `[x]` Review-state-aware publication summaries can be written as JSON,
...
- `[x]` Reviewed publication outputs can be packaged as
...
- `[x]` Atlas review readiness and the filterable review queue surface narrative
  ...
  before publication.
- `[x]` Atlas narrative review decisions can include reviewer identifiers and
...
- `[x]` Atlas Narrative review panels disclose when additional publication
...
- `[x]` Job and narrative API summaries expose compact reviewed publication
...
feature until the reviewed generation and publication workflow is polished in
the app surface.
...
3. Stabilize narrative generation metadata and the reviewed narrative report
   bundle contract.
