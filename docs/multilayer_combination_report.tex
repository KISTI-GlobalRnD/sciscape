...
The 6.5\% of 3-layer edges, consensus to $3\times$ weight, dominate Leiden's objective function and form cluster nuclei. Single-layer edges become relatively weak, preventing the mega-cluster absorption observed with simple summation.

%% ─────────────────────────────────────────────────────────
\section{Discussion}

\paragraph{Why consensus works.} CPM Leiden optimizes $H = \sum_c \left[e_c - \gamma \binom{n_c}{2}\right]$, where $e_c$ is internal edge weight. Consensus consensus amplifies edges that multiple independent measures agree on, concentrating weight in true communities. This creates a natural hierarchy: multi-layer cores attract single-layer periphery.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Consensus consensus sum} $(\sum w \times n_{\text{layers}})$ for multi-layer agreement.
