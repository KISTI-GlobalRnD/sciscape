...
\title{\textbf{Boosted Multi-Layer Edge Combination\\for Scientific Paper Clustering}}
...
\begin{algorithm}[h]
\caption{Boosted Multi-Layer Combination}
\begin{algorithmic}[1]
...
\ENDFOR
\STATE $E_{\text{combined}} \leftarrow \textsc{BoostedSum}(\{E_\ell''\})$ \COMMENT{Eq.~\ref{eq:boosted}}
...
\subsection{Boosted Consensus Sum (Step 3)}

\begin{equation}\label{eq:boosted}
    w_{ij}^{\text{boost}} = \left(\sum_{\ell=1}^{L} w_{ij}^{(\ell)}\right) \times n_{ij}
\end{equation}
...
\paragraph{Combination:} sum ($\sum w$), max ($\max w$), vote (count), boosted ($\sum w \times n$).

\subsection{Evaluation}

\begin{enumerate}[nosep]
    \item \textbf{Structural}: cluster count, max cluster \%, size distribution balance.
    \item \textbf{LLM blind review} (GPT-4o-mini, 55 cases): three criteria on disagreement nodes between sum and boosted:
    \begin{itemize}[nosep]
...
\textbf{1/rank} & \textbf{boosted} & \textbf{298} & \textbf{2.5} \\
...
$1/\text{rank}$ with sum or boosted produces the most balanced distributions. Vote is invariant to normalization (binary). Raw sum creates dominant clusters.

\subsection{Sum vs. Boosted: LLM Evaluation}
...
\begin{tabular}{lccc}
\toprule
Criterion & Sum & Boosted & Better \\
\midrule
Belonging (wins, $n{=}55$) & 26 & \textbf{29} & Boosted \\
...
Boosted assigns boundary nodes more accurately. Internal cluster quality is equivalent.

\paragraph{By cluster size bucket (30 cases):}
\begin{itemize}[nosep]
    \item \textbf{Large ($>$500)}: Sum 7:3 --- sum's mega-clusters win by volume.
    \item \textbf{Medium (200--500)}: Boosted 6:4 --- consensus correctly separates.
...
\subsection{Boosted Mechanism Analysis}

The boosted/sum weight ratio is exactly $n_{ij} \in \{1, 2, 3, 4\}$:
...
The 6.5\% of 3-layer edges, boosted to $3\times$ weight, dominate Leiden's objective function and form cluster nuclei. Single-layer edges become relatively weak, preventing the mega-cluster absorption observed with simple summation.

%% ─────────────────────────────────────────────────────────
\section{Discussion}

\paragraph{Why boosted works.} CPM Leiden optimizes $H = \sum_c \left[e_c - \gamma \binom{n_c}{2}\right]$, where $e_c$ is internal edge weight. Boosted consensus amplifies edges that multiple independent measures agree on, concentrating weight in true communities. This creates a natural hierarchy: multi-layer cores attract single-layer periphery.

\paragraph{Embedding layer integration.} Embedding $k$-NN edges have universal coverage (avg degree $\approx 50$) unlike citation layers (DC avg degree $= 5$ in humanities). Na\"ively adding Emb to boosted increases $n_{ij}$ globally, collapsing the differentiation. The solution is \textbf{not} to exclude Emb but to \textbf{automatically adjust $\gamma$}: auto-gamma search absorbs the density shift, letting Emb fill citation gaps while boosted consensus preserves cluster structure.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Boosted consensus sum} $(\sum w \times n_{\text{layers}})$ for multi-layer agreement.
