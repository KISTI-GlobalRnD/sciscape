"""Interactive HTML dashboard for keyword extraction results.

Generates a self-contained single-page application with:

- Cluster selector dropdown controlling all panels
- Tab 1: Keyword ranking (horizontal bar chart)
- Tab 2: Temporal trends (interactive line chart)
- Tab 3: Depth hierarchy (sunburst chart)
- Tab 4: Co-occurrence network (force-directed graph)
- Tab 5: Merge / normalization dictionary (searchable table)
- Tab 6: Cross-cluster heatmap (shared terms across clusters)
- Global keyword search with cross-cluster results
- CSV export for all tabs
- Emerging / declining keyword highlights in temporal tab

Main entry point::

    from sciscape.keyword_extraction.visualization import export_dashboard
    export_dashboard(result_df, "output/keyword_dashboard.html")

Optional dependency: ``pip install plotly``
"""
...
def _compute_network_edges(
    ...
    for i, t1 in enumerate(term_list):
        ...
        for t2 in term_list[i + 1:]:
            ...
            # Subphrase containment bonus
            containment = 0.0
            if w1 < w2 or w2 < w1:  # strict subset
                containment = 0.3
...
def _prepare_cluster_data(
    ...
    # Index viz_data by cluster for quick lookup
    ...
    # Group subphrase tree by cluster
    ...
    for cid, grp in df.groupby("cluster_id"):
        ...
        for _, r in grp_sorted.iterrows():
            ...
            # Temporal data — all available metrics
            ...
            # Backward compat: "temporal" alias for pub_year_series
...
# ---------------------------------------------------------------------------
# Individual chart functions (for programmatic use)
# ---------------------------------------------------------------------------
...
.header{background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;padding:1.5rem 2rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;}
.header h1{font-size:1.5rem;font-weight:700;}
.header .stats{display:flex;gap:1.5rem;font-size:.9rem;opacity:.85;}
.header .stats span{font-weight:600;color:#00d4ff;}
...
.footer{text-align:center;padding:2rem;font-size:.85rem;color:#6c757d;}
...
<div class="header">
  <h1>{{TITLE}}</h1>
  <div class="stats">
    <div>Keywords: <span>{{N_KEYWORDS}}</span></div>
    <div>Clusters: <span>{{N_CLUSTERS}}</span></div>
  </div>
</div>
...
<div class="footer">Generated by <b>SciScape</b> — Center for Global R&amp;D, KISTI</div>

<script>
const DATA = {{DATA_JSON}};

// Extract global data
const VOCAB_MERGES = DATA["_vocab_merges"] || {};
const NORM_MERGES = DATA["_norm_merges"] || {};
const CROSS_CLUSTER_TERMS = DATA["_cross_cluster_terms"] || null;
const TREND_SCORES = DATA["_trend_scores"] || {};
const CENTRALITY = DATA["_centrality"] || {};
const PIPELINE_CONFIG = DATA["_pipeline_config"] || {};
...
const clusterIds = Object.keys(DATA).map(Number).filter(n => !isNaN(n)).sort((a,b) => a-b);
...
if (clusterIds.length) {
  ...
  if (!restored) {
    clusterSelect.value = clusterIds[0];
    currentClusterId = clusterIds[0];
    currentCluster = DATA[clusterIds[0]];
    updateMeta();
  }
  updateScopeUI();
  renderCurrentTab();
...
def export_dashboard(
    ...
    viz_data : dict, optional
        ...
        Provides real cooccurrence edges, merge dictionaries, and
        subphrase hierarchy.
    ...
    cluster_data = _prepare_cluster_data(df, viz_data=viz_data)
