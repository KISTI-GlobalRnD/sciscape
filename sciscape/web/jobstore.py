...
class JobStore:
        ...
        with self._lock:
            ...
            if self._use_sqlite():
                row = self._conn.execute(
                    "SELECT status, request, result, progress FROM jobs WHERE job_id = ?",
                    (job_id,),
                ).fetchone()
                if row is not None:
                    job = {
    ...
    def persist(self, job_id: str) -> None:
        ...
        with self._lock:
            ...
            if job and self._use_sqlite():
                self._conn.execute(
                    "UPDATE jobs SET status=?, result=?, progress=? WHERE job_id=?",
                    (job["status"],
        ...
        with self._lock:
            if self._use_sqlite():
                rows = self._conn.execute(
                    "SELECT job_id, status, request, result, progress FROM jobs"
                ).fetchall()
                return [
    ...
    def list_all(self) -> List[Dict[str, Any]]:
        with self._lock:
            if self._use_sqlite():
                rows = self._conn.execute(
                    "SELECT job_id, status, request FROM jobs ORDER BY created_at DESC"
                ).fetchall()
                return [
                    {"job_id": r[0], "status": r[1], "query": json.loads(r[2] or "{}").get("query", "")}
                    for r in rows
                ]
            else:
                return [
